...
@objc(BookCore_RNAARViewController)
public class RNAARViewController: UIViewController, PlaygroundLiveViewMessageHandler, PlaygroundLiveViewSafeAreaContainer, ARSCNViewDelegate {
    ...
    public override func viewDidLoad() {
        ...
        self.view.insertSubview(ARView, belowSubview: statusLabel)
        ...
        ARView.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 0).isActive = true
        ARView.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: 0).isActive = true
...
//        sceneView.allowsCameraControl = false
    ...
    func insertGeometry(hitResult: ARHitTestResult) {
        ...
        let scene = SCNScene(named: "RNA.scn")
        guard let DNANode = scene?.rootNode.childNode(withName: "RNA-1", recursively: true) else { return }
    ...
    public func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        DispatchQueue.main.async {
            self.statusLabel.text = "Found a surface! Tap to add RNA!"
        }
    }
